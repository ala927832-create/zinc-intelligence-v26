...
def _close_charts(close: dict) -> str:
    ...
    return f"""<div class='chart-title'><b>長期 Close＋SMA14／SMA30／EMA20</b><span>{len(points)} plotted sessions</span></div>
    ...
    <svg viewBox='0 0 {width} 290' role='img' aria-label='Long-term LME zinc 3M reference Close with moving averages'><rect x='{left}' y='18' width='{plot_w}' height='{plot_h}' class='chart-frame'/>{ticks}{paths}<text x='{left}' y='283' class='chart-label'>{_esc(points[0]['date'])}</text><text x='{width-right}' y='283' text-anchor='end' class='chart-label'>{_esc(points[-1]['date'])}</text></svg>
...
.close-chart{{margin-top:14px}}.close-chart svg{{width:100%;height:auto;display:block;margin:4px 0 12px}}.chart-frame{{fill:#071827;stroke:#1c4668}}.chart-grid{{stroke:#173b59;stroke-width:1}}.chart-line{{fill:none;stroke-width:2;stroke-linejoin:round;stroke-linecap:round}}.chart-line.close{{stroke-width:2.8}}.chart-line.rsi{{stroke:#5cd4ff}}.rsi-band{{stroke:#ffb253;stroke-width:1;stroke-dasharray:5 5}}.chart-label{{fill:#8ca9c1;font-size:11px}}.chart-title,.chart-legend{{display:flex;justify-content:space-between;gap:12px;flex-wrap:wrap;margin:13px 0 6px}}.chart-title span,.chart-legend{{color:#8ca9c1;font-size:11px}}.chart-legend{{justify-content:flex-start}}.range-line{{stroke:#5cd4ff;stroke-width:3;opacity:.7}}.range-close{{fill:#ffb253}}
